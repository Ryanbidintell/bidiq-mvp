...
- **Follow-up automation (LARGELY BUILT, admin-gated — not unstarted):** Cialdini-based AI drafting, OAuth send (Gmail + M365), customizable sequence templates. Schema, drafting engine, send path, OAuth scaffolding, and full UI exist on disk and are gated to admins (`isAdmin()`). Outcome capture is via the user answering "Has the GC responded?" before each send. ⚠️ **There is NO AI parsing of GC replies and NO auto-assignment of replies to projects** — that capability was never in the build spec and does not exist in code (verified 2026-06-29; see `VERIFICATION_2026-06-29.md`). Pulled forward from later phasing because it drives stickiness and gates partnership outreach.
...
- [~] **Follow-up automation** — *ships before the BuildingConnected API.* **Largely BUILT + admin-gated** (verified 2026-06-29). Cialdini-based AI drafting, OAuth send (Gmail + M365), customizable sequence templates, full UI, and 5 DB tables all exist. Outcome capture = user-answered "Has the GC responded?" prompt. **NOT built: AI parsing of GC replies → auto-update outcomes** (never spec'd; the "AI response parsing" line was aspirational). Two known cron bugs — see KNOWN_BUGS.md. Pricing gating: Solo = 15 active schedules/mo + 1 cloned template; Team/Company = unlimited; founders (FOUNDING30/FOUNDER30) = unlimited forever. Spec: `BidIntell_FollowUp_Automation_Build_Spec_v1.md`.
- [~] **Scope-filtering / CSI false-positive fix** — **Largely DONE** (verified 2026-06-29). The Bid Risk scope-vagueness scan (`vague_scope`) runs on trade-targeted pages via `selectTargetPages(allPages, docMap, userCSISections)`, not the full document — so it already filters to the user's selected CSI sections. Residual gaps: (1) `vague_scope` is a coarse boolean with no per-section attribution; (2) when targeting matches <5 pages (user has no CSI sections, or thin doc map) it falls back to the first 20 pages, where the old false-positive vector resurfaces. Smallest tightening step documented in `VERIFICATION_2026-06-29.md`.
