-- ═══════════════════════════════════════════════════════════════════
-- BIDINTELL MINIMAL MIGRATION - Only Add Missing Columns
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'user_settings' AND column_name = 'decision_time'
    ) THEN
        ALTER TABLE user_settings ADD COLUMN decision_time TEXT DEFAULT 'normal';
        COMMENT ON COLUMN user_settings.decision_time IS 'How much time user takes for bid decisions: quick, normal, or thorough';
        RAISE NOTICE 'Added decision_time column to user_settings';
    ELSE
        RAISE NOTICE 'decision_time column already exists in user_settings';
    END IF;
END $$;

-- Add constraint for decision_time
ALTER TABLE user_settings DROP CONSTRAINT IF EXISTS valid_decision_time;
ALTER TABLE user_settings ADD CONSTRAINT valid_decision_time
...
-- Add constraint for ai_advisor_tone
