...
- `scripts/eval-extraction.js` + `eval/golden/*.json` - Extraction/scoring ACCURACY harness (hand-labeled real bids → per-field accuracy, recommendation-band match, score delta). The `test-*.js` scripts check engine LOGIC; this measures real-world accuracy + guards against model drift. Run: `node scripts/eval-extraction.js [--min 85]`. Add bids via `eval/README.md`.
