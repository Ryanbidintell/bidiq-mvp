...
### UI/UX Improvements
- **Full-Page Report View:** Replaced cramped modal popup with dedicated full-screen report page featuring editable fields, complete AI analysis, and integrated actions (View/Outcome/Delete)
- **Projects Table Enhancement:** Improved button layout with horizontal flexbox, icons (👁️ View, 📝 Outcome, 🗑️ Delete), wider Actions column (20%), and right-aligned buttons for better UX
- **Editable Report Details:** Users can now edit project name, building type, location, GC, bid deadline, and analysis date directly from the report view with save functionality

### Geographic & Location Features
- **County-Based Location Support:** System now detects and geocodes "County, State" format (e.g., "Johnson County, KS") for rural projects that don't have specific city addresses
- **Improved Distance Calculations:** County center coordinates used when city is unavailable, enabling accurate distance scoring for all project types

### Data Structure & Compatibility
- **Multi-Field GC Name Support:** System checks multiple possible field names (`gc_names`, `general_contractor`, `gc_name`) to ensure GC information displays correctly regardless of data structure
- **Building Type Normalization:** Handles both object and string formats for building_type field
- **Architect Field Filtering:** Automatically filters out service taglines (e.g., "Architecture, Planning, Interiors, Programming") and shows "Not found" instead of incorrect data
- **Building Square Footage Display:** Properly formats and displays project_size_sf with commas (e.g., "25,000 SF")

### Technical Improvements
- **Error Handling in Report View:** Comprehensive try-catch blocks with detailed error logging for debugging
- **Database Field Mapping:** Fixed field name consistency between database (`created_at`, `extracted_data`) and in-memory representations (`createdAt`, `extracted`)
- **Save Functionality:** Report edits properly save to database with correct field names and update GC database with new entries

### Bug Fixes
- **Fixed GC Score Display:** GCs with star ratings but no wins now show correct scores (uses rating × 20 instead of 0% win rate)
- **Fixed Report Navigation:** Resolved `showTab` vs `switchTab` function name issue preventing report view from displaying
- **Fixed Tab Switching:** Removed inline `display: none` style that was overriding CSS class-based tab switching
- **Fixed Date Handling:** Properly handles both `created_at` and `createdAt` field names throughout the codebase

---

*[All v1.6 content preserved below]*
...
**County-Based Location Support (NEW v1.7):**
- System detects "County, State" format (e.g., "Johnson County, KS")
- Geocodes county center when city address unavailable
- Enables accurate distance calculations for rural projects
- Progressive fallback strategy: Full address → City, State → County, State
...
## FULL-PAGE REPORT VIEW (NEW v1.7)

### The Problem
Modal popup was cramped and limited - users couldn't see complete analysis or edit project details without navigating away.

### The Solution: Dedicated Full-Screen Report Page

**Features:**
- **Editable Project Details Card:**
  - Project Name (text input)
  - Building Type (dropdown)
  - City, State (text inputs with auto-uppercase)
  - Bid Deadline (date picker)
  - General Contractor (text input, auto-adds to GC database)
  - Analysis Date (date picker, can backdate)

- **Complete AI Analysis Display:**
  - Overall BidIndex Score with recommendation
  - All 4 scoring components with full details
  - AI recommendations and action items
  - Extracted data grid

- **Integrated Actions:**
  - 💾 Save Changes button (persists edits to database)
  - 📝 Add Outcome button (opens outcome modal from report)
  - ← Back button (returns to previous tab)

**Navigation Flow:**
- Click "View" from Recent Activity or Projects table
- Full-page report opens with all details
- Edit any fields as needed
- Save changes (updates database + GC database)
- Add outcome directly from report
- Back button returns to previous view

**Technical Implementation:**
- New `tab-report` section in main layout
- `viewReport()` function populates report page instead of modal
- `saveReportEdits()` handles database updates
- `closeFullReport()` returns to previous tab with state preservation
- Automatic refresh when outcome added from report view

**Why This Matters:**
- Better UX - more space to see complete analysis
- Inline editing - faster workflow for corrections
- Integrated actions - all functions available in one place
- Professional appearance - feels like real business software
- Improved data quality - easier to correct extraction errors
...
| **Architect name** | Display (filtered for service lists in v1.7) |
| **Owner name** | Display |
...
|---------|---------|---------|---------|
| Office location | City, State, Street, ZIP | Required | Distance calculation |
...
- ✅ **Company type selection** — Subcontractor / Distributor / Manufacturer Rep **(NEW v1.6)**
- ✅ **Automated onboarding sequence** — adapts questions per company type (NEW v1.5)
- ✅ User preferences with all settings including **location importance slider**
- ✅ **Product lines / brands onboarding** for distributors and mfg reps **(NEW v1.6)**
- ✅ **Keywords as separate section** with clear UI
- ✅ **AI-powered contract risk detection** (automatic) **with confidence weighting (NEW v1.6)**
- ✅ Multi-GC selection before analysis (from **Master GC List**)
- ✅ **Duplicate project detection** — same project from multiple GCs creates one entity with per-GC scores **(NEW v1.6)**
- ✅ **Master GC database with admin dashboard**
- ✅ **GC name normalization agent** (AI-powered real-time matching + admin review queue) **with alias system (NEW v1.6)**
- ✅ **Custom GC risk tags** (user-created, admin-promoted)
- ✅ PDF upload and AI extraction (Claude API)
- ✅ **Multi-signal trade detection** — CSI headers, drawing sheet prefixes (M-, E-, P-), material keywords, drawing titles **(NEW v1.6)**
- ✅ **Product Match scoring** for distributors/mfg reps — Specified/Approved/Or Equal/Not Listed detection **(NEW v1.6)**
- ✅ **Building type extraction** (hospital, office, multifamily, etc.)
- ✅ **County-based location support** — geocodes county center when city unavailable **(NEW v1.7)**
- ✅ 4-component personalized scoring (BidIndex Score) **with component adaptation per company type**
- ✅ Plain-English report generation **with score data lineage / source linking (NEW v1.6)**
- ✅ **Report language adapts per company type** (trade match vs product match) **(NEW v1.6)**
- ✅ **Full-page report view** with editable fields and integrated actions **(NEW v1.7)**
- ✅ **"How to Improve Your Chances" section on every report**
- ✅ Dashboard with stats
- ✅ **Bid counter/ticker** on dashboard
- ✅ **Beta feedback widget** — in-app form, emails founder, tracked in dashboard **(NEW v1.6)**
- ✅ Projects list with enhanced UI **(NEW v1.7)**
...
- ✅ **Passive ghost trigger** — auto-marks stale projects as ghosted **(NEW v1.6)**
...
- ✅ **Multi-provider AI fallback** — Claude → OpenAI with error notifications **(v1.7)**
...
- **v1.7** - February 6, 2026 - UI/UX enhancements + geographic flexibility + data resilience:
  - **Full-Page Report View** (replaces modal popup with dedicated full-screen report page)
  - **Editable Report Details** (inline editing of project name, building type, location, GC, dates with save functionality)
  - **County-Based Location Support** (detects and geocodes "County, State" format for rural projects)
  - **Projects Table UI Enhancement** (horizontal buttons with icons, flexbox layout, 20% Actions column)
  - **Multi-Field GC Name Support** (checks `gc_names`, `general_contractor`, `gc_name` for display compatibility)
  - **Building Type Normalization** (handles both object and string formats)
  - **Architect Field Filtering** (removes service taglines, shows clean data or "Not found")
  - **Building Square Footage Display** (properly formatted with commas: "25,000 SF")
  - **Multi-Provider AI Fallback** (Claude → OpenAI with error notifications documented)
  - **Fixed GC Score Display** (GCs with ratings but no wins now show correct scores)
  - **Comprehensive Error Handling** (try-catch blocks throughout report view)
  - **Database Field Mapping Fixes** (consistent handling of `created_at`/`createdAt`, `extracted_data`/`extracted`)
  - Added `street` and `zip` columns to `user_settings` table
