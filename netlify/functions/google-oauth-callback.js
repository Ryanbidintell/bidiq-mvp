...
// Handles the Google OAuth redirect after user consents to gmail.send scope.
//
// Flow:
//   1. Google redirects here with ?code=...&state=...
//   2. We HMAC-verify the state to recover user_id
//   3. POST to Google token endpoint to exchange code → access + refresh tokens
//   4. Fetch user's email address from Google userinfo endpoint
//   5. Encrypt tokens via oauth-shared/token-crypto
//   6. Upsert into user_email_integrations
//   7. Redirect browser back to /app.html?gmail_connected=true
//
// SECURITY:
...
function redirectWithError(reason) {
  return {
    statusCode: 302,
    headers: { Location: `${APP_URL}?gmail_error=${encodeURIComponent(reason)}` },
    body: '',
  };
}
...
exports.handler = async function (event) {
  ...
  if (oauthError) {
    // e.g. user clicked "deny" on Google consent screen
  ...
  // Verify HMAC-signed state and extract user_id
  let userId;
  try {
    ...
    console.error('google-oauth-callback — state verification failed:', e.message);
  ...
  try {
    ...
    console.error('google-oauth-callback — token exchange error:', e.message);
    return redirectWithError('token_exchange');
  }

  const { access_token, refresh_token, expires_in, scope: grantedScope } = tokenData;
  if (!access_token || !refresh_token) {
    // Missing refresh_token usually means user previously consented and we
    // didn't pass prompt=consent — our start fn always does, so this should
    // be rare. Treat as a hard error so user can re-attempt cleanly.
    console.error('google-oauth-callback — token response missing access_token or refresh_token');
    return redirectWithError('token_incomplete');
  }

  // Fetch the user's email address from Google's userinfo endpoint so we
  // know the From: address for outbound emails. Don't trust the user to type
  // their own — pull it from the consented account.
  ...
  try {
    ...
    console.error('google-oauth-callback — userinfo fetch failed (non-fatal):', e.message);
  }
  if (!emailAddress) {
  ...
  try {
    ...
    console.error('google-oauth-callback — encryption failed:', e.message);
  ...
  // Upsert on user_id (table has UNIQUE constraint) — reconnecting replaces the row
  try {
    ...
    console.error('google-oauth-callback — DB save error:', e.message);
    return redirectWithError('db_save');
  }

  return {
    statusCode: 302,
    headers: { Location: `${APP_URL}?gmail_connected=true` },
    body: '',
  };
};
