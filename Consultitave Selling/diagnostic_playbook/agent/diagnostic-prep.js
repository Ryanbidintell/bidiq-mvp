...
async function sendFounderEmail({ subject, body, driveFolderUrl }) {
  const recipient = process.env.DIAGNOSTIC_NOTIFY_EMAIL || 'ryan@fsikc.com';
...
exports.handler = async (event) => {
  ...
  // We import findExistingProspectFolder lazily to keep the rest of the code path clean.
  try {
    ...
    // If the early check itself fails, log and proceed — we'll fall through to the normal path
  ...
  try {
    ...
    if (folderAlreadyExisted) {
      // Webhook retry — folder already exists, skip writes to avoid duplicates
