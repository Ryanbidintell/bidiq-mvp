...
const BUILDING_TYPES = {
    ...
    OFFICE: {
        ...
        keywords: ['office', 'corporate', 'headquarters', 'business park', 'professional building', 'tenant improvement'],
        icon: '🏢'
    },
    MULTIFAMILY: {
...
function matchBuildingTypeByKeywords(projectName, scopeSummary) {
    ...
    // Check each building type for keyword matches
    let bestMatch = null;
    let highestScore = 0;
    ...
    Object.entries(BUILDING_TYPES).forEach(([key, type]) => {
        ...
        if (matchCount > highestScore) {
            ...
            bestMatch = {
                ...
                label: type.label,
                icon: type.icon,
                confidence: Math.min(matchCount * 0.3, 0.9), // Max 0.9 for keyword matching
                method: 'keyword'
            };
        }
    });
