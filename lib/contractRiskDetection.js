...
function formatContractRisksForDisplay(risks) {
    ...
    let html = `
        <div style="margin-top:16px;padding:16px;background:var(--bg-secondary);border-radius:8px;border-left:4px solid var(--${risks.overallRiskLevel === 'high' ? 'danger' : risks.overallRiskLevel === 'medium' ? 'warning' : 'info'});">
            <div style="font-weight:600;margin-bottom:8px;display:flex;align-items:center;gap:8px;">
                <span>⚠️ Contract Risks Detected (${risks.risksDetected.length})</span>
                <span style="font-size:11px;padding:2px 8px;background:var(--bg-tertiary);border-radius:4px;">Overall: ${risks.overallRiskLevel.toUpperCase()}</span>
            </div>
