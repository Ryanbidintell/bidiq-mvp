...
//
// NAMED BUILDING_TYPE_LIST, not BUILDING_TYPES, on purpose. app.html loads this
// file AND lib/buildingTypeExtraction.js as plain <script> tags, so both share one
// global scope — and buildingTypeExtraction.js already declares a (different,
// object-shaped) top-level `const BUILDING_TYPES`. Two top-level `const`s of the
// same name is a SyntaxError that aborts this ENTIRE file, so none of the v2
// scoring functions below ever got defined in the browser. That in turn made
// scoreV2Enabled() in app.html — which guards on
// `typeof scoreOpportunity === 'function'` — silently return false forever, so the
// BidIndex v2 preview and its divergence shadow-logging never ran on the app path.
// Keep these two names distinct. The module.exports key below is still
// BUILDING_TYPES, so Node consumers and the edge copy are unaffected.
const BUILDING_TYPE_LIST = ['Healthcare','Office','Multifamily','Retail','Industrial','Education','Higher Education','Government','Religious','Mixed-Use','Infrastructure','Other'];
...
if (typeof module !== 'undefined' && module.exports) {
    // Export key stays BUILDING_TYPES so Node consumers (scripts/test-scoring-core.js)
    // and the edge scoring-core.ts copy keep the same public API after the rename.
    module.exports = { scoreOpportunity, scoreProjectFit, scoreScopeFit, scoreClientFit, keywordModifier, outlierContractPenalty, computeCompleteness, recommend, BUILDING_TYPES: BUILDING_TYPE_LIST, CONTRACT_CLAUSE_TIER, ABSTAIN };
}
